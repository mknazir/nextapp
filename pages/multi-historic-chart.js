...
const MultiChart = () => {
    ...
    const fetchHistoricalData = async () => {
        ...
        displayedCoins.forEach((coin) => {
            intervals.forEach((interval) => {
                const historicalDataUrl = `https://api.binance.com/api/v3/klines?symbol=${coin}&interval=${interval}&endTime=${dateTimeInSeconds}`;
                requests.push(
    ...
    return (
        <div className='coin-container'>
            <Tabs onSelect={(index) => setSelectedTab(displayedCoins[index])} className="tabContainer" style={{ width: "100%" }}>
